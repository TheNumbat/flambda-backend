...
let close_c_call acc env ~loc ~let_bound_ids_with_kinds
  ...
  let box_return_value =
    ...
    | _, Unboxed_vector (Pvec128 ty) ->
      Some (P.Box_number (Naked_vec128 ty, Alloc_mode.For_allocations.heap))
  ...
  let kind_of_primitive_native_repr
    ...
    | Unboxed_vector (Pvec128 ty) -> K.naked_vec128 ty
  in
  let param_arity =
    ...
    List.fold_left2
      (fun (call : Simple.t list -> Acc.t -> Expr_with_acc.t) arg
        ...
        let unbox_arg : P.unary_primitive option =
          ...
          | _, Unboxed_vector (Pvec128 ty) ->
            Some (P.Unbox_number (Naked_vec128 ty))
        in
