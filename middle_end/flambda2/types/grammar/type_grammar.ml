...
type t =
  ...
  | Naked_vec128 of Primitive.vec128_type * head_of_kind_naked_vec128 TD.t
...
and head_of_kind_value =
  ...
  | Boxed_vec128 of Primitive.vec128_type * t * Alloc_mode.For_types.t
  | Closures of
...
and free_names_head_of_kind_value0 ~follow_value_slots head =
  ...
  | Boxed_vec128 (_, ty, _alloc_mode) -> free_names0 ~follow_value_slots ty
...
and apply_renaming_head_of_kind_value head renaming =
  ...
  | Boxed_vec128 (vty, ty, alloc_mode) ->
    let ty' = apply_renaming ty renaming in
    if ty == ty' then head else Boxed_vec128 (vty, ty', alloc_mode)
...
let rec print ppf t =
  ...
  | Naked_nativeint ty ->
    Format.fprintf ppf "@[<hov 1>(Naked_nativeint@ %a)@]"
      ...
      ty
  | Naked_vec128 (vty, ty) ->
    Format.fprintf ppf "@[<hov 1>(Naked_vec128[%s]@ %a)@]"
      (Primitive.vec128_name vty)
...
and print_head_of_kind_value ppf head =
  ...
  | Boxed_nativeint (ty, alloc_mode) ->
    Format.fprintf ppf "@[<hov 1>(Boxed_nativeint@ %a@ %a)@]"
      Alloc_mode.For_types.print alloc_mode print ty
  | Boxed_vec128 (vty, ty, alloc_mode) ->
    Format.fprintf ppf "@[<hov 1>(Boxed_vec128[%s]@ %a@ %a)@]"
      (Primitive.vec128_name vty)
      Alloc_mode.For_types.print alloc_mode print ty
  | Closures { by_function_slot; alloc_mode } ->
...
and ids_for_export_head_of_kind_value head =
  ...
  | Boxed_vec128 (_, t, _alloc_mode) -> ids_for_export t
  | Closures { by_function_slot; alloc_mode = _ } ->
...
let kind t =
  ...
  | Naked_vec128 (ty, _) -> K.naked_vec128 ty
...
let alias_type_of (kind : K.t) name : t =
  ...
  | Naked_number (Naked_vec128 ty) -> Naked_vec128 (ty, TD.create_equals name)
...
let bottom_naked_vec128 ty = Naked_vec128 (ty, TD.bottom)
...
let any_naked_vec128 ty = Naked_vec128 (ty, TD.unknown)
...
module Head_of_kind_value = struct
  ...
  let create_boxed_vec128 vty ty alloc_mode = Boxed_vec128 (vty, ty, alloc_mode)
