...
module Expanded_type : sig
  ...
  type descr =
    ...
    | Naked_vec128 of Primitive.vec128_type * TG.head_of_kind_naked_vec128
  ...
  let create_naked_vec128 ty head =
    { kind = K.naked_vec128 ty; descr = Ok (Naked_vec128 (ty, head)) }
  ...
  let of_non_alias_type ?coercion ty : t =
    ...
    | Naked_vec128 (ty, Unknown) -> create_unknown (K.naked_vec128 ty)
    | Naked_vec128 (ty, Bottom) -> create_bottom (K.naked_vec128 ty)
    | Naked_vec128 (ty, Ok (No_alias head)) -> (
      ...
      | Some coercion -> (
        ...
        | Bottom -> create_bottom (K.naked_vec128 ty)
  ...
  let descr_oub t : descr_oub =
    ...
    | Unknown -> (
      ...
      | Naked_number (Naked_vec128 ty) -> Naked_vec128 (ty, Unknown)
    ...
    | Bottom -> (
      ...
      | Naked_number (Naked_vec128 ty) -> Naked_vec128 (ty, Bottom)
